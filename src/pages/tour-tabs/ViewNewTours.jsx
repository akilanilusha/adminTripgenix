...
export default function ViewNewTours() {
  ...
  async function loadAll() {
    ...
    //console.log("Booking:", driverRes.data);
  ...
  //console.log("Booking:", selectedDriver);
...
async function confirmByTripGenix() {
  const toastId = toast.loading("Confirming trip...");

  try {
    await bookingApi.confirmByTripGenix(id);
    ...
    const message =
      ...
      "Failed to confirm trip. Please try again.";
  ...
  return (
    <div className="p-6 space-y-6">
      <PageBreadcrumb
        title="View Booking"
        paths={["Tours", data.referenceId, "View"]}
      />
      ...
      <div className="space-y-10">
        ...
        <Section title="Booker Details">
          <Grid>
            <ReadOnly
              label="Booker Name"
              value={data.bookingDetails.nameOfBooker}
            />
            <ReadOnly label="Email" value={data.bookingDetails.bookerEmail} />
            <ReadOnly label="Phone" value={data.bookingDetails.bookerPhone} />
            <ReadOnly
              label="Flight Number"
              value={data.bookingDetails.flightNumber}
            />
            <ReadOnly
              label="Passport"
              value={data.bookingDetails.passportNumber}
            />
            <ReadOnly
              label="Arrival Airport"
              value={data.bookingDetails.departureAirport}
            />
            <ReadOnly
              label="Adults"
              value={data.bookingDetails.passengers.adults}
            />
            <ReadOnly
              label="Children"
              value={data.bookingDetails.passengers.children}
            />
            <ReadOnly
              label="Babies"
              value={data.bookingDetails.passengers.babies}
            />
          </Grid>
        </Section>
        ...
        <Section title="Trip Details">
          ...
          <ReadOnly
            ...
            value={data.tripDetails.startLocation}
          />
          <ReadOnly
            full
            label="End Location"
            value={data.tripDetails.endLocation}
          />
          ...
          {selectedVehicle ? (
            <div className="border p-4 rounded-sm space-y-1">
              ...
              <p>
                ...
                {Number.isFinite(selectedVehicle.passengerCount)
              ...
              <p>
                ...
                {Number.isFinite(selectedVehicle.costPerKm)
                  ? `LKR ${Number(selectedVehicle.costPerKm).toFixed(2)}`
                  : "—"}
              </p>

              <p>
                <span className="font-medium">Booking Price:</span>{" "}
                {Number.isFinite(selectedVehicle.bookingPrice)
                  ? `LKR ${Number(selectedVehicle.bookingPrice).toFixed(2)}`
                  : "—"}
              </p>
            </div>
          ) : (
